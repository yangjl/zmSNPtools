...
# of its sequence
#
# Change Log v(2.3) - 2014.04.09 - Eddy
#  o Increase default quality threshold for end and middle sequences to PHRED Q=20 (<=1% error rate)
#
# Change Log (v2.2) - 2013.08.27 - Eddy
#  o Script no longer carries the descriptions from sequence identifiers
#
# Change Log (v2.1) - 2013.06.07 - Eddy
#  o Program now adds corresponding /1 and /2 tags to the name by scanning the second field of
#    the sequence description
#
# Change Log (v2.0) - 2013.03.06 - Eddy
#  o Allow reading of fastq sequences with length zero (empty sequence)
#
...
use constant VERSION => "2.3 (2014.04.09)";
use constant true => 1;
use constant false => 0;
use constant DEFAULT_MIN_EQ_QUALITY => 20;		# Default minimum quality threshold at the ends of the read
use constant DEFAULT_MIN_IQ_QUALITY => 20;		# Default minimum quality threshold in the middle of the read
...
# Fixes sequence id names if read name does not end with /1 or /2 and description is either 1:XYZ or 2:XYZ
sub fixSequenceID {
    my @tmp = split(/\s/, $_[0]);
    my $id;

    if (defined($tmp[1]) && length($tmp[1]) != 0 && $tmp[0] !~ m/\/(1|2)$/) {
        if ($tmp[1] =~ m/^1:/) {
            $id = sprintf("%s/1", $tmp[0]);
        } # end of if statemetn
        elsif ($tmp[1] =~ m/^2:/) {
            $id = sprintf("%s/2", $tmp[0]);
        } # end of else if statement
    } # end of if statement

    #$id = $_[0] if (!defined($id));
    $id = $tmp[0] if (!defined($id));	# change in v2.2

    return $id;
} # end of sub fixSequenceID
...
sub nextFastqSequence {
    ...
    while (!$stop && !eof($fh)) {
        ...
        if ($line =~ m/^(@\S+)/) {   # found sequence descritor
            # $seq_desc = $1;		
			$seq_desc = &fixSequenceID($line);	# Change in v2.1
            $stop = true;
        } # End of if statement
    } # End of while loop
    ...
    while (!$stop && !eof($fh)) {
        ...
        if ($line =~ m/^(\+\S{0,})/) {  # found quality descriptor
            # $qual_desc = $1;
			$qual_desc = &fixSequenceID($line);	# Change in v2.1
            $stop = true;
        } # end of if statement
    } # End of while loop
...
sub printLogStats {
	...
	print $log sprintf("# TOTAL BASE PAIRS REMAINING (POST-TRIM): %s bp (%2.1f%%)\n", &formatNumber($total_bp_post_trim), ($total_bp_post_trim / $total_bp) * 100); 
	print $log sprintf("#\n");
