...
then
	echo ""
	echo "ERROR: Please specify path to the directory containing 'phase I' files"
	echo ""
	exit 0;
else 
	# Create symbolig links
	find $1 -name "*.gte300.fas" -exec ln -s '{}' . \;
	find $1 -name "*.gte300.*.gff3" -exec ln -s '{}' . \;

	for prefix in `find -name "*.gte300.fas" | awk '{ sub(/\.gte300\.fas$/, "", $0); print $0; }' | sort -u`
	do
