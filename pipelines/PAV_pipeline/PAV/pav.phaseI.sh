...
then
	...
	echo "   ARGUMENT 1: Top level directory containing ABySS assembled contigs"
	...
	# Copy *.gte300.fas from assembly folder to current directory
	find $dir -name "$sample.*.gte300.fas" -exec cp '{}' . \;

	for prefix in `find "$sample".*.gte300.fas | awk '{ sub(/\.fas$/, "", $0); print $0; }' | sort -u`
	do
		if [ ! -e "$prefix".length ]
		then
