...
###################################
#
# like modifList, but works when names of val are unique prefixes of names of x
#
...
###################################
#
UnrecognizedItems <- function(list,names,message="\tUnrecognized: ",verbose=TRUE) {
	w <- which (! names(list) %in% names);
	if (length(w) > 0 && verbose) { 
		message(paste(message,names(list)[w],"=", list[[w]], "\n")); 
	};
	return( names(list)[w] );
}
...
AdjustModesOfArgs <- function(args) {
		args <- sublapply(args,
				c('legendAlpha', 'width','height',
					...
					'ymax','legendSize','refsnpTextSize','axisSize','axisTextSize','geneFontSize','smallDot',
					'largeDot','refDot'),
				as.numeric);

		args <- sublapply(args,
				c('chr','rfrows','unit','xnsmall'),
				as.integer);
...
###################################
#
# Converter from chrom to chr format
#
...
###################################
#
# Converter from chr to chrom format
#
...
# Tailored to fit Peters nominclature and formatting.
#
...
grid.refsnp <- function(name,pos) {
	...
	grid.text(name,x=unit(pos,"native"), y=.95, hjust="center", vjust="top",
...
###################################
# functions for pasting grobs together and printing them
# this could be improved, but it suffices for the moment.
#
...
gpaste <- function(grobList,...,unitTo='npc',sep=NULL,
	x=unit(.5,'npc'),y=unit(.5,'npc'),just=c('left','bottom')){
	l <- length(grobList)
	if ( l <= 1 ) { return(grobList) }
	zList <- grobList
	if ( ! is.null (sep) ) {
		sepGrob <- textGrob(sep,...)
		zList <- list(grobList[[1]])
		if (l > 1) {
			for (i in 2:l) { 
				zList[[2*i - 2]] <- sepGrob
				zList[[2*i - 1]] <- grobList[[i]]
				}
			grobList <- zList
		}
	}
	w <- lapply(grobList,grobWidth,...)
	widths <- lapply(w,convertX,unitTo=unitTo)
	totalWidth <- unit(sum(unlist(widths)),unitTo)
	startX <- x;
	if (just[1] == 'center') { startX <- startX - .5*totalWidth }
	if (just[1] == 'right') { startX <- x - totalWidth }

	widthSoFar <- unit(0,unitTo)
	for ( i in 1:length(grobList) ) {
		gg <- editGrob(grobList[[i]], x=startX + widthSoFar,y=y,
				just=c('left',just[2]))
		grid.draw(gg)
		#grid.rect(width=grobWidth(gg),height=grobHeight(gg),
		#				x=unit((i+2)/20,'npc'))
		#
		widthSoFar <- widthSoFar + widths[[i]]
	}
	return(grobList)
}
...
###################################
make.gene.list <-  function (flat, showIso=TRUE, ascii=TRUE, subset, unit, ...) 

{       
		if ( prod(dim(flat)) <= 0 ) { return(""); }
		...
		if (ascii) {
			...
			return(paste( df0$idnum, df0$name, df0$chrom, df0$start, df0$stop, sep=",", collapse="\n" ));
		} else {
			return("");
		} # end else not ascii
}
...
#		if (is.character(args[['requiredGene']])) {  # put requiredGene first in list
#			df0 <- df0[order(df0$name != args[['requiredGene']]),]    
#		}
	...
	# conversion to 'native' isnot working, so we convert evertyhing via inches to npc below.	
	# conversion utility
		...
		for (i in 1:dim(df0uniq)[1]) {
			...
			# temp debugging info
			#print(c(left=left,right=right,leftGraphic=leftGraphic,rightGraphic=rightGraphic,leftName=leftName,rightName=rightName,rowToUse))
		}
...
###################################
#
# Zoomplot from metal output (with positions added), ld (ala newfugue), and recombination rate data
# 
...
########## annotation (genes)
	if(1) {
		...
		grid.text(paste('position on',chr2chrom(args[['chr']]),unit2char(args[['unit']])), 
			y=unit(args[['xlabPos']],'lines'),vjust="bottom",
			gp=gpar(cex=args[['axisTextSize']], col=args[['axisTextColor']], alpha=args[['frameAlpha']]) 
			);
...
#    labels=c(labels,"SNP set(s)");
#    values=c(values,args[['snpset']]);
#    labels=c(labels,"# of SNP sets");
#    values=c(values,format(nrugs));
    ...
    if(FALSE) {  # (is.null(args[['reload']]) || debug){
        labels=c(labels,"ld");
        values=c(values,args[['ld']]);
        labels=c(labels,"annot");
        values=c(values,args[['annot']]);
        labels=c(labels,"metal");
        values=c(values,args[['metal']]);
        labels=c(labels,"recomb");
        values=c(values,args[['recomb']]);
    }

#    labels=c(labels,"all data saved in");
#    values=c(values,args[['rdata']]);

#    labels=c(labels,"metal data saved in");
#    values=c(values,args[['metalcsv']]);

#    labels=c(labels,"plot saved in");
#    if ( 'png' %in% args[['format']] ) {
#    	values=c(values,args[['png']]);
#    } else {
#    	values=c(values,args[['pdf']]);
#    }
#
    labels=c(labels,"number of SNPs plotted");
    values=c(values,as.character(dim(metal)[1]));

    labels=c(labels,"best p-value");
	bestIdx <- which.max(transformation(metal$P.value));
	bestName <- as.character(metal$MarkerName[bestIdx]);
	bestNegLogP <- transformation(metal$P.value[bestIdx]);
	bestPSci <- log2sci(-bestNegLogP)
    values=c(values,paste(bestPSci," [", bestName ,"]",sep=""));

#    labels=c(labels,"-log10(p-value) range");
#    values=c(values,paste("(",format(min(transformation(metal$P.value))),", ",format(max(transformation(metal$P.value))),")",sep=""));

#    labels=c(labels,"p-value cut-off");
#    values=c(values,format(args[['thresh']]));

#    labels=c(labels,"prelude");
#    values=c(values,args[['prelude']]);

#    labels=c(labels,"postlude");
#    values=c(values,args[['postlude']]);
	if (TRUE) { 
		oG <- omittedGenes;
		while (length(oG) > 0) {
...
#	transformPvalue=TRUE,                 # should we apply -log10 to the p-values?
...
ld.default <- data.frame(snp1=NA, snp2=NA, build=NA, chr=NA, pos1=NA, pos2=NA, 
				midpoint=NA, distance=NA, rsquare=NA, dprime=NA, r2dp=NA)[c(),,drop=FALSE]
...
# if (! is.null(user.args[['requiredGene']]) ){
#	default.args[['title']] <- gpaste(gList(
#			textGrob(user.args[['requiredGene']]), 
#			textGrob(' region')
#		))
#} 
...
# UnrecognizedItems(args, names(default.args), "Unrecognized argument: ");
...
# metal not saving as csv anymore
# args <- ModifyList(
# 	list( metalcsv = paste(args[['prefix']],"_metal", ".csv", sep="") ),
# 	args 
#    );
...
if(0) {
		s <- recrate$pos >= args[['startBP']] & 
			 recrate$pos <= args[['endBP']] & 
			 recrate$chr == args[['chr']];
		recrate <- subset(recrate, s);
}
		...
		if (! is.null(ld)) {
			  ...
			  if (length(snpCols) != 2) {
				warning("LD file doesn't smell right.")
			  ...
			  if (prod(dim(lld)) > 0) { 
					  metal <- merge(metal, lld,  
						...
						metal$group[metal$MarkerName == refSnp] <- length(args[['ldColors']]) #2 + length(levels(metal$ldcut));
				} else {
		...
		command <- paste("pquery refFlat_in_region",
				" -defaults",
				" -sql",
				" Chrom=", chr2chrom(args[["chr"]]),  #  chr",args[["chr"]],
				" Start=",args[["start"]],
				" End=",args[["end"]],
				" Build=",args[["build"]],
				sep="");
...
#save(metal,recrate,refFlatRaw,refFlat,rug,file="debug.Rdata");
...
# yRange <- range(transformation(metal$P.value),na.rm=T);
yRange <- c(0,max(c(args[['ymax']],transformation(metal$P.value)*1.1),na.rm=T));
...
#if (!args[['showAnnot']] || !args[['showRefsnpAnnot']]) {
