...
p2g <- function (predict, train) {
  ...
  # Update date: 8/25/09
  #===========================================================================================#
  library(mgcv, lib="~/bin/Rlib")
  # read marker file:
  # marker<-read.delim(predict,header=T)
  marker <- predict
  # read P-G data:
  #d<-read.csv(train,header=T) # subject to change
  d <- train
  ...
  # output
  #write.table(marker_Pos,paste(predict,".gen",sep=""),quote=F,sep="\t")
  return(marker_Pos)
}